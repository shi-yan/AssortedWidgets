...
namespace OGLFT {
  ...
  void Face::setStringRotation ( GLfloat string_rotation )
  {
	...
	float angle;
	if ( string_rotation_ < 0. ) {
	  angle =static_cast<float>( 360. - fmod( fabs( string_rotation_ ), 360.f ));
	}
	else {
...
//	       foreground_color_[A] );
  ...
  void Face::draw ( GLfloat x, GLfloat y, const char* s )
  {
	...
	dx = -bbox.x_min_; break;
      case CENTER:
	dx =static_cast<GLfloat>( -( bbox.x_min_ + bbox.x_max_ ) / 2.); break;
      case RIGHT:
	...
	dy = -bbox.y_min_; break;
      case MIDDLE:
	dy = static_cast<GLfloat>(-( bbox.y_min_ + bbox.y_max_ ) / 2.); break;
      case TOP:
...
//	       foreground_color_[A] );
  ...
  BBox Raster::measure ( unsigned char c )
  {
    ...
    gluUnProject( bbox.x_min_, bbox.y_min_, 0., modelview, projection, viewport,
		  &x, &y, &z );
    bbox.x_min_ =static_cast<float>( x - x0);
    bbox.y_min_ =static_cast<float>( y - y0);

    gluUnProject( bbox.x_max_, bbox.y_max_, 0., modelview, projection, viewport,
		  &x, &y, &z );
    bbox.x_max_ =static_cast<float>( x - x0);
    bbox.y_max_ =static_cast<float>( y - y0);

    gluUnProject( bbox.advance_.dx_, bbox.advance_.dy_, 0., modelview, projection,
		  viewport,
		  &x, &y, &z );
    bbox.advance_.dx_ = static_cast<float>(x - x0);
    bbox.advance_.dy_ = static_cast<float>(y - y0);
  ...
  void Raster::setRotationOffset ( void )
  {
    ...
    rotation_offset_y_ =static_cast<GLfloat>( rotation_reference_face_->glyph->bitmap.rows / 2.);
  }
  ...
  void Monochrome::renderGlyph ( FT_Face face, FT_UInt glyph_index )
  {
    ...
    glBitmap( bitmap_glyph->bitmap.width, bitmap_glyph->bitmap.rows,
	      ...
	      static_cast<float>(bitmap_glyph->bitmap.rows - bitmap_glyph->top),
	      static_cast<float>(face->glyph->advance.x / 64.),
	      static_cast<float>(face->glyph->advance.y / 64.),
	      inverted_bitmap );
  ...
  void Grayscale::renderGlyph ( FT_Face face, FT_UInt glyph_index )
  {
    ...
    glBitmap( 0, 0, 0, 0,
	      static_cast<float>(bitmap_glyph->left),
	      static_cast<float>(bitmap_glyph->top - bitmap_glyph->bitmap.rows),
	      0 );
    ...
    glBitmap( 0, 0, 0, 0,
	      static_cast<float>(-bitmap_glyph->left + face->glyph->advance.x / 64.),
	      static_cast<float>(bitmap_glyph->bitmap.rows - bitmap_glyph->top +
  ...
  void Polygonal::setRotationOffset ( void )
  {
    ...
    rotation_offset_y_ =static_cast<float>(
  ...
  void Outline::renderGlyph ( FT_Face face, FT_UInt glyph_index )
  {
    ...
    if ( character_rotation_.active_ ) {
      glPushMatrix();
      glTranslatef(static_cast<float>( ( face->glyph->metrics.width / 2. +
		      face->glyph->metrics.horiBearingX ) / 64.
		    * vector_scale_),
		    static_cast<float>(rotation_offset_y_),
		    0.0f );
    ...
    glTranslatef( static_cast<float>(face->glyph->advance.x / 64. * vector_scale_),
		  static_cast<float>(face->glyph->advance.y / 64. * vector_scale_),
		  0.0f );
...
/*
  ...
  void Texture::setRotationOffset ( void )
  {
    ...
    rotation_offset_y_ = static_cast<float>(rotation_reference_face_->glyph->bitmap.rows / 2.);
  }
  ...
  void Texture::renderGlyph ( FT_Face face, FT_UInt glyph_index )
  {
    ...
    if ( character_rotation_.active_ ) {
      glPushMatrix();
      glTranslatef( static_cast<float>(( texture_info.width_ / 2. +
		      texture_info.left_bearing_ )),
		    static_cast<float>(rotation_offset_y_), 0.0f );
    ...
    glTexCoord2i( 0, 0 );
    glVertex2f( static_cast<float>(texture_info.left_bearing_),static_cast<float>( texture_info.bottom_bearing_ ));

    glTexCoord2f( texture_info.texture_s_, 0. );
    glVertex2f( static_cast<float>(texture_info.left_bearing_ + texture_info.width_),
		static_cast<float>(texture_info.bottom_bearing_ ));

    glTexCoord2f( texture_info.texture_s_, texture_info.texture_t_ );
    glVertex2f( static_cast<float>(texture_info.left_bearing_ + texture_info.width_),
		static_cast<float>(texture_info.bottom_bearing_ + texture_info.height_ ));

    glTexCoord2f( 0., texture_info.texture_t_ );
    glVertex2f( static_cast<float>(texture_info.left_bearing_),
		static_cast<float>(texture_info.bottom_bearing_ + texture_info.height_ ));
    ...
    // Drawing a character always advances the MODELVIEW.
    glTranslatef( static_cast<float>(texture_info.advance_.x / 64.),
		  static_cast<float>(texture_info.advance_.y / 64.),
		  0.0f );
  }
